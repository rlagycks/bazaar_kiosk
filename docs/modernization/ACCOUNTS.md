...
## PR #67 리뷰 결과 (2026-09-20)

코드·보안·DB 리뷰 에이전트 3개. CRITICAL/HIGH 없음. 남긴 것:

- **배포 창(MEDIUM):** 0025는 한 트랜잭션이라 `orders_authdevice`의 ACCESS EXCLUSIVE 잠금이 마이그레이션 끝까지
  유지된다. 그동안 모든 인증 요청이 대기한다(격리 fixture, 주문 20만 건에서 총 0.8초, 최대 대기 306ms).
  어차피 모든 기기를 회수하므로 **운영 트래픽 없는 창에서 적용**한다. 데이터가 크면 `lock_timeout`을 두고 재시도한다.
- **포장 목록 anti-join(MEDIUM):** 포장 모니터링 목록은 `DINE_IN` 항목 전체를 훑어 제외한다(20만 건에서 5.4ms).
  누적 이력이 커져 눈에 띄면 `OrderItem(order_id) WHERE service_mode='DINE_IN'` 부분 인덱스나 서버 측 기본
  상태·날짜 경계를 추가한다. 목록 GET의 쿼리 수는 3개로 변함없다.
- LOW: 항목이 0개인 주문은 포장으로 분류된다(API로는 만들 수 없고 관리자 인라인 삭제로만 가능). 관리자 인라인에서
  진행 이력이 있는 항목을 지우면 PROTECT가 500으로 드러난다. 이름은 유니코드 정규화를 하지 않는다(관리자 등록 계정).
