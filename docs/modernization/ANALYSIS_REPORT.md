...
- 심각도/상태: **Critical / Repo-fixed(익명 차단·CSRF 면제만)**. 아래는 수정 이전 관찰이다.
  **매출 기밀성 항목은 아직 수정되지 않았다.** `stats-dashboard`는 제한됐으나 같은 숫자가
  `orders-collection` GET·`order-detail`로 나간다. [등록부](RISK_REGISTER.md) 참조.
