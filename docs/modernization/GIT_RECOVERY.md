...
준비 작업으로 변경된 tag, 기본 브랜치 또는 보호 설정은 없습니다.
원격 ref는2026-09-10에 한 번 변경했습니다. 머지 완료된 현대화 작업 브랜치7개를 사용자 지시로 삭제했고,
각 팁이 해당 PR의 머지 시점 head와 같은지 대조한 뒤 실행했습니다. 팁 SHA는 [작업 로그](WORKLOG.md)에 있습니다.
위6번이 권고하는 보관 tag는 만들지 않았습니다. 대신 `refs/pull/<번호>/head`가 원격에 남아 있어
`git fetch origin refs/pull/46/head` 형태로 복구할 수 있습니다. 과거 사용자 브랜치는 삭제하지 않았습니다.
