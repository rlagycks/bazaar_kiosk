...
상태: 구현 설명. 운영 배포 완료 또는 미결정 정책에 대한 사용자 승인을 의미하지 않는다.
D-035의 공용 ID/비밀번호 계정, D-042/043의 토큰 수명·갱신 구조를 다룬다.
...
- DB 폐기 상태, 계정 공급 상태, ID 및 자격증명 fingerprint를 access·refresh·HTML
  ...
  계정의 기존 디바이스를 모두 무효화한다. **비밀번호 교체 시 전체 디바이스를 종료하는
  운영 정책은 사용자 확인 대상이며, 이 구현 설명이 승인을 대신하지 않는다.**
...
| `LOGIN_MAX_FAILURES` | 배포자가 명시해야 하는 양수. 미입력 기본 0은 로그인 사용 불가 상태 |
| `LOGIN_WINDOW_SECONDS` / `LOGIN_BLOCK_SECONDS` | 현재 코드의 후보 값은 각각 300초. 운영 정책 확정과 구분 |

ID는 고유해야 하고 앞뒤 공백 없이 최대 128자다. 비밀번호는 평문 환경변수 대신 Django
`pbkdf2_sha256` 인코딩 해시를 공급한다. 샘플·테스트 계정과 실패 횟수 값은 합성 fixture이며
운영 자격증명이나 승인된 정책이 아니다. 실제 실패 제한 횟수와 잠금 정책은 운영 전 확정한다. 사용자 답변이 남은 항목은
비밀번호 교체 시 전체 디바이스 종료, 5분 동안 5회 실패 시 5분 잠금 제안, 메뉴·테이블 조회를
인증된 전 계정에 허용하는 범위다. 현재 구현·유지된 동작을 승인된 기본 정책으로 읽지 않는다.
