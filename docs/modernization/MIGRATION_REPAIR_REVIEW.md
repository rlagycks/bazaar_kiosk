...
`gh pr list --state merged`로 확인한다.8A는2B 이후 최종 인수 대기이고 현재80개(15+65)를 PG에서 검증한다.
4A1은 오류 보고 가림과 운영 필수 설정 시작 거부까지 진행했다. BK-R028 종료는 운영 인수 대기다.
...
**해소(2026-09-12, D-037):** 사용자가 보존할 과거 주문 데이터가 없음을 확정했다.
빈 DB 신규 설치에서는 아래 표의 마지막 후보(**정책 확정 전 원본0019 유지**)가 그대로 결과가 되고
나머지 다섯 개는 성립하지 않는다. 적용된 migration을 수정하지 않는다는 보호는 그대로 유지된다.
표는 위반 행이 존재할 때의 선택지 기록으로 남긴다.
운영 DB에 데이터가 발견되면 이 해소는 무효이며 다시 판단해야 한다.
