...
운영 배포·4A2 전체 완료·최종 머지를 주장하지 않습니다. `LOGIN_MAX_FAILURES`는 운영 환경에
양수로 명시해야 하며 테스트 설정은 합성 값입니다. 비밀번호 변경 시 전체 디바이스 종료,
5분 5회/5분 잠금, `menus` 쓰기 전 계정 허용은 사용자 답변을 기다리는 정책입니다.
