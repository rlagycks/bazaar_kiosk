...
| 2 | [BK-R035 — 자체 SSE 실행에 필요한 비동기 경로·워커 조건 미확립](ANALYSIS_REPORT.md#bk-r035) | High | Reproduced | 완화 (10A, D-057: 실제 uvicorn+nginx에서 프레임 순차 수신·일반 API 병행·DB 연결 0·끊김 시 스레드/FD 복귀·정지 10.5초 측정. 동기 WhiteNoise 미들웨어 제거와 시스템 검사로 재발 차단. **스트림 1개=스레드 1개**와 브라우저 경로는 열림) | D-006,D-018,D-019; BK-R001/019 | 실시간·운영 담당 | 10 | 실제 ASGI HTTP 스트림·일반 API 병행, 동기 middleware 적응/스레드·워커 수·disconnect/reload 정리 |
