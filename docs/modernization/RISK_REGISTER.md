...
API9개가 익명 포함 모든 행위자에게 열려 있고 쓰기3개는 CSRF 토큰 없이도 통과한다.
[측정과 D-003 결정표](AUTHORIZATION_MATRIX.md). 인가를 강제하는 코드는 변경하지 않았다.
증거가 보강됐을 뿐 위험은 Critical·Open 그대로이며 D-003 확정과 단계3 구현이 남는다.
D-031 외부 접속 확정으로 이 위험의 "외부 노출 범위" 미확인 항목이 노출 상향 방향으로 해소됐다.

D-034~039 후속(2026-09-12): 인증·배포·데이터 전제가 확정됐다. **어느 위험도 종료하지 않는다.**
- D-037로 보존 대상 과거 주문이 없음을 확인했다. BK-R017의 과거행 제약 호환 조건이 성립하지 않아
  범위가 빈 DB 신규 설치로 좁아진다. 이는 조건 소멸이지 검증 완료가 아니므로 해결 상태는 Open이다.
...
단계3 후속(2026-09-13): BK-R001의 API 인가와 CSRF 경계를 저장소에서 수정하고 회귀13개로 고정했다.
익명은 전 구간 차단, `order-status`·`order-item-progress`는 주방, 매출 통계 조회는 주방 카운터다.
[구현과 증거](API_AUTHORIZATION.md). 인증 방식은 바꾸지 않았으므로 공용 PIN 약점(BK-R002)은 그대로다.
브라우저 실제 여정과 401/403 구분은 미검증이며 외부 노출·배포는 미실행이다.

**증거 상태는 부분 수정이다. 세 항목 중 두 개만 닫혔다.**
- 익명 접근 차단: 수정됨. 전 API에서 403이다.
- 변경 API의 CSRF 면제: 수정됨. 쓰기3개가 토큰을 요구한다.
- **매출 기밀성: 수정되지 않았다.** `stats-dashboard`를 카운터로 제한했으나 같은 숫자가
  `orders-collection` GET과 `order-detail`로 새어 나간다. `_serialize_order`가 주문마다
  `total_price`·`payment_method`·현금/식권 구성·거스름돈을 담고 이 두 엔드포인트는 인증만 요구한다.
  D-040이 이 주체를 정하지 않아 추측으로 좁히지 않았는데, 여기서는 "정하지 않음"이 중립이 아니라
  이미 내린 제한을 무효로 만든다. **D-040 후속 결정 없이는 이 항목을 닫을 수 없다.**

독립 리뷰(2026-09-13)가 인증 측 약점도 지적했다. 세션이 이제 9개 API의 유일한 인가 자격증명이라
전보다 무겁다. 세션 고정(로그인 시 `cycle_key()` 없음), 토큰 없는 쓰기가 CSRF middleware에서
HTML403으로 거부되는 점, 역할 회수가 기존 세션에 닿지 않는 점, CSRF 가능한 GET 로그아웃이다.
모두 인증 코드 변경이므로 4A2의 별도 승인 사안이며 이 단계에서 수정하지 않았다.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | [BK-R001 — API 역할 인가 부재와 변경 API CSRF 면제](ANALYSIS_REPORT.md#bk-r001) | Critical | Repo-fixed(익명 차단·CSRF만) | Open (매출 기밀성 미해결, D-040 후속 필요) | D-036,D-040; 단계 2 | 보안 담당 | 3 | 모든 역할/익명/만료 세션, CSRF 없음·오류·유효, GET/POST/PATCH/HEAD 경계. **매출 필드가 `orders-collection`으로 노출됨. 브라우저 여정·401/403 미검증** |
