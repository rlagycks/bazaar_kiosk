...
- D-036이 익명 차단·CSRF 면제 제거·JSON403을 확정해 BK-R001의 개선 방향이 정해졌다.
  구현은 D-003 확정 후이며 BK-R001은 Critical·Open 그대로다.
- D-035의 쿠키 기반 JWT는 CSRF 위험을 되살리므로 BK-R001의 CSRF 항목은 인증 전환 후에도 유효하다.
...
- D-038로 배포 대상이 EC2로 확정됐다. BK-R043/R044의 운영 대상 검증 조건이 구체화될 뿐
  검증 자체는 수행하지 않았다.
- D-038 조사에서 `origin/main`이 빈 DB에서 실패하는 수정 전 `0020`을 담고 있었음을 확인했다.
  develop 정렬 전까지 main 배포는 BK-R005를 그대로 재현했을 것이다. 정렬 PR은 별도다.
