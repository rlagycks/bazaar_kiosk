...
migration15개·앱/guard65개, 총80개·skip0을 실행한다. 새6개는 DB 접근이 없는 SimpleTestCase이며
...
**D-035 연동 주의.** 지역변수 가림은 `orders/views/auth.py`의 `pin`·`expected`라는 이름에 묶여 있다.
역할별 계정 인증으로 전환하며 자격증명 지역변수 이름이 바뀌면 그 가림은 멈춘다.
POST 필드는 이름 패턴 대조라 비교적 견디지만(`password`도 덮는다),
D-035의 id/password + JWT 전환에서 이 두 이름은 사라진다. 구현 시 데코레이터 인자와
이 문서의 경계를 함께 갱신해야 하며, 그러지 않으면 가림이 조용히 멈추고 현재 회귀는 모두 통과한다.
...
4A1 전체 인수에 필요한 운영 설정 누락 거부는 2026-09-14에 구현했다. [구현과 증거](REQUIRED_SETTINGS.md).
운영 실제 값 확인은 여전히 남는다.
