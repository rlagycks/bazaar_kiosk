...
- **D-042로 네 가지를 확정했다.** 전부 사용자가 권장안을 선택했다.
...
- 거부 메시지는 환경 변수 이름만 말하고 값은 넣지 않는다. 이 예외는 오류 보고에 도달할 수 있고
  값은 BK-R028이 거기 나타나면 안 된다고 한 바로 그것이다. 누락 항목은 한 번에 모두 보고한다.
...
- 회귀10개. 모두 새 인터프리터를 띄워 실제로 시작해 보고 결과를 읽는다.
...
- **기존 테스트 두 곳이 이 변경으로 깨졌고, 한 곳은 조용히 잘못된 이유로 통과할 뻔했다.**
  `settings_test_pg`가 환경을 비우고 기본 설정을 별표 import 해서 `DEBUG` 명시가 필요했다.
  `test_database_config`의 탐침도 같은 이유로 실패했는데, 같은 파일의 다른 테스트는
  "오류가 났다"만 단언해서 **URL과 무관한 DEBUG 거부로도 통과**하고 있었다.
  거부 사유가 `DATABASE_URL`인지 단언하도록 좁혔다.
- 변이10개를 주입해 전부 사망을 확인했다. 전체75개(15+60) 통과, skip0. 소유 label 확인 후 정리했다.
