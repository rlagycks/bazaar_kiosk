...
### 2026-09-22 — UI-05C 최종 리뷰와 UI-11 통합 점검

- 사용자 지시: 남은 통계와 통합 UI 점검을 각각 PR로 제출하고 모든 PR에 서브에이전트
  리뷰를 수행한다. Figma 05안+확정 댓글을 유지하며 merge·배포는 하지 않는다.
- UI-05C [PR83](https://github.com/rlagycks/bazaar_kiosk/pull/83): develop `e61d6ab`
  대비 최종 head `9517bfc` 독립 리뷰 APPROVE, 지적 없음. GitHub CI 성공 확인.
- UI-11 브랜치 `ui/05-ui-acceptance`는 PR83에서 분리했다. PR83이 열린 상태이므로 후속
  PR의 기준은 `ui/05-stats-dashboard`이며 합병 순서는 PR83→UI-11이다. `ui/**` 대상 PR에도
  CI가 실행되도록 workflow 필터를 추가했다. 매 PR의 독립 리뷰 규칙을 AGENTS에 기록했다.
- 사용자 기기 답변: Chrome 기준, Safari도 접속 가능. 실제 기기 인수를 미실행으로 남기고
  [UI_ACCEPTANCE](UI_ACCEPTANCE.md)에 재현 절차·결과·잔여 인수를 분리했다.
- UI 수정: 결제창의 수량/삭제/수량 표시와 모니터링 준비 수량 버튼의 접근성 이름에
  홀·포장 구분을 넣었다. 같은 메뉴가 양쪽에 있을 때 버튼 목록만으로 구별되지 않던 문제다.
  시각 디자인·API·DB·금액/상태 규칙은 바꾸지 않았다. 회귀 2개 red→green을 확인했다.
- 서브에이전트가 실제 PostgreSQL·CSRF·4개 별도 권한 계정으로 3개 업무 통합 테스트를
  추가했다. 혼합/포장 범위, 부분 준비, 낡은 버전 거부, 명시적 출발, 재개·취소와 매출,
  권한 거부 후 데이터 불변 및 연습 매출 제외를 실제 HTTP 경계에서 검증한다.
- 자동 검증: PostgreSQL 마이그레이션 27 + 앱 598 = **625 통과**, system check 정상,
  migration drift 없음. CI와 같은 Node 검사 **100 통과**. 로그는 무시되는
  `.venv/ui11-pg.log`, `.venv/ui11-node.log`, `.venv/ui11-labels-{red,green}.log`에 남겼다.
- Codex 내장 브라우저의 실제 앱 점검:
  - 서빙 390×844·320×568·390×400, 메뉴 10개. 결제 내부 스크롤과 고정 저장 접근,
    가로 넘침 없음, Escape 후 초안 유지, 잘못된 테이블 오류 초점·입력 보존 후 저장.
  - 홀 김밥2+포장 김밥1/국수1, 5,000원(현금4,000/식권2,000/거스름1,000/순현금3,000).
    식당에서 부분 준비1/4 후 명시적 출발4/4, 통계1건/4개와 수납 내역 대조.
  - 두 번째 포장 주문1,000원은 포장 화면에만 표시. 출발 후 전체 내역에서 둘을 확인하고,
    첫 주문을 준비 완료·출발 대기로 복귀해 수량 보존 확인 후 취소. 취소 상세 수정 차단,
    통계1건/1개/매출1,000/현금2,000/거스름1,000/순현금1,000/취소1건 확인.
  - PC1440×1000·1024×768, 키보드 Enter 준비 수량/삭제, Tab 이동, 내 메뉴 Escape 초점
    복귀·로그아웃 확인. 서빙 전용에는 주문·서빙만 표시, 통계 직접 접근은 로그인 복귀.
  - 실제 모바일 Chrome/Safari, 소프트 키보드, VoiceOver/TalkBack은 미실행이다.
    viewport·DOM 접근성 검사로 실기기 인수나 전체 단계11 완료를 주장하지 않는다.
- 검증 도구 `scripts/ui_acceptance_preview.py`: 검증된 전용 제어 DB 아래 UUID fixture,
  합성 계정/메뉴/테이블, localhost만 사용한다. 리뷰에서 활성 SSE가 종료를 지연하는 점을
  발견했다. 단일 프로세스에 3초 제한만 두었을 때 UUID DROP에 열린 연결이 남는 실패를
  재현했고, spawn 서버 프로세스의 완전한 종료 뒤 부모가 DB를 제거하도록 보완했다.
  실제 실시간 연결 상태에서 Ctrl-C 종료0과 해당 UUID 제거를 확인했다. 다른 DB/세션의
  강제 제거는 사용하지 않았다. 후속 리뷰에서 연속 Ctrl-C로 정리가 중단되는 경로도 보완했다.
  정리 구간의 추가 SIGINT를 무시하고 DB 제거 후 신호 처리기를 복원하며, 실제 SSE 활성
  상태에서 연속 Ctrl-C 두 번도 종료0·UUID 잔존0건으로 확인했다. 근거는
  `.venv/ui11-preview-shutdown{,-fixed}.log`, `.venv/ui11-preview-double-interrupt.log`다.
- 격리·정리: Compose `bk-ui11-0922`, loopback55465 PG15. 임시 서버·브라우저 탭·viewport,
  해당 Compose 컨테이너/볼륨을 정리했다. 운영/기존 개발 데이터에는 접근하지 않았다.
- 남은 범위: 실제 Chrome 기본 인수와 Safari 호환성·음성 접근성, 10E 부하 검증, 12 배포.
