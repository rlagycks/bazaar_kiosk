...
현재2A·1A·PR40은 머지,1B는0019 정책 대기입니다.8A는 PR42 머지 후2B 이후 최종 인수 대기이며
D-029 PostgreSQL 전용 실행/CI는 PR44로 머지됐습니다.4A1의 오류 보고 PIN 가림은 PR46으로,
D-031/D-032 결정 기록은 PR48로, 단계3 준비 측정은 PR52로 머지됐습니다.
2026-09-12에 D-034~D-040을 기록했습니다. 주방 계정 단일화, JWT 인증, 익명 차단과
CSRF 면제 제거 준비, 과거 데이터 없음, EC2 배포, 설정 누락 시작 거부, 3계정 권한 매트릭스입니다.
모두 **결정이며 구현은 시작 전**입니다. 운영 시작 정책은 D-039로 확정됐고 2026-09-14에 구현했습니다.
2B·4A1 전체 종료는 남아 있습니다.
**승인된** 새 작업은 매 세션 확인한 최신 develop에서 분기합니다. 열린 PR·기준 ref를 먼저 조회합니다.
...
진행할 수 있습니다.로컬 PG·CI 기반 검증만으로 운영·원본 데이터 관문을 면제하지 마세요.
...
BLUEPRINT와 SESSION_SETUP의 격리 검증 프로필을 사용하세요. D-029로 SQLite 지원을 제거했습니다.
POSTGRES_TESTING의 전용 Compose 준비·정리와 scripts/test_postgres.py 전체 검증을 따르세요.
명시적 테스트 URL과 실제 DB identity/권한을 확인하고 migration/app을 별도 프로세스로 실행합니다.
운영/개발 DB를 테스트 대상으로 쓰거나 DB 충돌 시 자동 삭제하거나 실패를 skip/fake로 감추지 마세요.
CI는 같은 PostgreSQL 명령을 사용합니다. 2B 전체·원본 운영 경로는 별도 종료 관문입니다.
