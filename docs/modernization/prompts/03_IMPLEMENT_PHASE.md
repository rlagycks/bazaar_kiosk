...
2026-09-07 청사진 재검토 반영: 상위 단계 전체가 아니라 `2A`, `1B`, `10D1` 같은
하위 단계 하나를 선택합니다. 첫 구현 후보는 `2A`입니다. 아래 템플릿 자체나 청사진 문서
완료만으로 앱 구현·운영 이전이 자동 승인되지는 않습니다.
...
BLUEPRINT의 검증 프로필을 사용하세요.2A가 만들 settings_test는 합성 자격증명과 메모리
SQLite를 강제하고 운영 DATABASE_URL이나 기존 db.sqlite3를 사용하지 않아야 합니다.
1A가 만들 settings_test_pg는 별도 테스트 URL과 실제 DB/호스트/권한을 검증해야 합니다.
예정된 테스트 모듈이나 설정 파일이 아직 없으면 해당 단계에서 구현하고 실제 수집을 확인하세요.
일반 manage.py를 기본 운영 설정으로 실행하거나 테스트0개를 성공으로 보고하지 마세요.
